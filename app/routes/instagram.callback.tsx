...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  if (!payload?.shop) {
    return {
      ok: false as const,
      message:
        "This link is invalid or has expired. Start the connection again from Dream Reels.",
    };
  }
  ...
  try {
    ...
    return {
      ok: false as const,
      message:
        "Could not complete the connection. Please try again from Dream Reels.",
    };
  }
};
  ...
  return (
    ...
    >
      ...
      <p style={{ margin: "0 0 20px", color: "#5c6b70" }}>
        {result.ok
          ? `Connected as @${result.username}. You can close this tab and return to Dream Reels to import your reels.`
          : result.message}
      </p>
      <button
        type="button"
