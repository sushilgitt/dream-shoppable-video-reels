...
docker run --name dreamreels-pg -e POSTGRES_PASSWORD=dreamreels \
  -e POSTGRES_DB=dreamreels -p 5432:5432 -d postgres:16
...
- [x] **Phase 2 — Video pipeline.** Bunny Stream ingest, transcode webhook,
      poster frames, library UI, plan-limit enforcement.
- [x] **Phase 3 — Instagram.** Facebook Login, Business account linking, reel
      sync, per-shop token refresh.
- [x] **Phase 4 — Tagging and widgets.** Product picker with timestamp ranges,
      six widget layouts, placement rules.
- [x] **Phase 5 — Storefront.** Theme app extension and the player itself.
- [x] **Phase 6 — Analytics and billing.** Event ingest, revenue attribution,
