...
(function () {
  ...
  var ORIGIN = "https://dreamreels.91.239.208.85.sslip.io";
  ...
  function session() {
    ...
    try {
      sessionKey = sessionStorage.getItem("dreamreels_s");
      if (!sessionKey) {
        sessionKey = Math.random().toString(36).slice(2) + Date.now().toString(36);
        sessionStorage.setItem("dreamreels_s", sessionKey);
      }
    } catch (error) {
  ...
  function stampCart(videoId, widgetId) {
    var attributes = {};
    attributes["_dreamreels_video"] = videoId;
    attributes["_dreamreels_widget"] = widgetId;
    attributes["_dreamreels_session"] = session();
  ...
  /** "Powered by Dream Reels" badge — free plan only. */
  function buildWatermark() {
    var link = el("a", "dreamreels__badge", {
      href: "https://dreamreels.91.239.208.85.sslip.io",
      target: "_blank",
      rel: "noopener nofollow",
    });
    link.textContent = "Powered by Dream Reels";
    return link;
  }
  ...
  function buildProducts(video, config, shop, widget) {
    ...
    var wrap = el("div", "dreamreels__products");
    var entries = [];

    video.products.slice(0, MAX_BUILT_PRODUCTS).forEach(function (product) {
      var card = el("div", "dreamreels__product");
      ...
      var text = el("div", "dreamreels__product-text");
      var title = el("span", "dreamreels__product-title");
      title.textContent = product.title;
      text.appendChild(title);

      var priceEl = el("span", "dreamreels__product-price");
      ...
      if (product.variantId) {
        var button = el("button", "dreamreels__cta", { type: "button" });
        ...
        var select = el("select", "dreamreels__variants", {
        ...
        var pickerButton = el("button", "dreamreels__cta", { type: "button" });
        ...
        var soleButton = el("button", "dreamreels__cta", { type: "button" });
        ...
        var link = el("a", "dreamreels__cta", { href: "/products/" + product.handle });
  ...
  function buildYouTubeTile(video, config, shop, widget) {
    var tile = el("div", "dreamreels__tile dreamreels__tile--embed", {
      "aria-label": video.title || "Shoppable video",
    });

    if (video.poster) {
      tile.appendChild(
        el("img", "dreamreels__media dreamreels__poster", {
          src: video.poster,
          alt: "",
          loading: "lazy",
          decoding: "async",
    ...
    var host = el("div", "dreamreels__media dreamreels__embed");
  ...
  function buildTikTokTile(video, config, shop, widget) {
    ...
    if (video.poster) {
      var poster = el("img", "dreamreels__media dreamreels__poster", {
        src: video.poster,
        alt: "",
        loading: "lazy",
        decoding: "async",
    ...
    tile._load = function () {
      ...
      // silently.
      frame = el("iframe", "dreamreels__media dreamreels__embed", {
        src:
  ...
  function buildHostedTile(video, config, shop, widget) {
    var tile = el("div", "dreamreels__tile", {
      role: "button",
      tabindex: "0",
      "aria-label": video.title || "Shoppable video",
    });
    ...
    var media = el("video", "dreamreels__media dreamreels__video", {
      playsinline: "",
      preload: "none",
      "aria-hidden": "true",
    });
  ...
  function applyConfig(root, config) {
    root.style.setProperty("--dreamreels-accent", config.accentColor);
    root.style.setProperty("--dreamreels-accent-text", config.accentTextColor);
    root.style.setProperty("--dreamreels-radius", config.cornerRadius + "px");
    root.style.setProperty("--dreamreels-ratio", ratioValue(config.aspectRatio));
    root.style.setProperty("--dreamreels-per-row", config.itemsPerRow);
  }
  ...
  function renderWidget(root, widget, shop, watermark) {
    ...
    if (config.heading) {
      var heading = el("h2", "dreamreels__heading");
    ...
    if (widget.layout === "STORIES") {
      var bar = el("div", "dreamreels__stories");
      widget.videos.forEach(function (video) {
        var story = el("button", "dreamreels__story", { type: "button" });
        if (video.poster) {
          story.appendChild(
      ...
      var list = el(
        "div",
        widget.layout === "GALLERY" ? "dreamreels__grid" : "dreamreels__rail"
      );
      widget.videos.forEach(function (video) {
  ...
  function openLightbox(video, config, shop, widget) {
    var overlay = el("div", "dreamreels__lightbox dreamreels");
    ...
    var close = el("button", "dreamreels__close", {
      type: "button",
      "aria-label": "Close",
    });
    close.textContent = "×";
  ...
  function renderFloating(root, widget, shop) {
    ...
    var host = el("div", "dreamreels__floating", {
    ...
    close.addEventListener("click", function (event) {
      ...
      host.remove();
      try {
        sessionStorage.setItem("dreamreels_float_closed", "1");
      } catch (error) {
  ...
  function mount(root) {
    ...
    loadPayload(shop).then(function (payload) {
      var skeleton = root.querySelector(".dreamreels__skeleton");
      ...
      due.forEach(function (widget) {
        if (widget.layout === "FLOATING") {
          var closed = false;
          try {
            closed = sessionStorage.getItem("dreamreels_float_closed") === "1";
          } catch (error) {
        /* storage blocked (private mode) — not worth surfacing */
      }
  ...
  function boot() {
    document.querySelectorAll("[data-dreamreels]").forEach(mount);
  }
